...
function ShopItem(props: any) {
  return (
    <Box key={props.item.id} component='div' display='flex' alignItems='center' flexWrap='wrap' py={1} rowGap={1}>
      <Typography component='span' display='block' width={200}>
        {props.item.name}
      </Typography>
      <Box component='div' pr={!props.isMobile ? 6 : 2}>
        <Button variant='outlined' disabled={props.item.inCart <= 0} sx={{ minWidth: 5 }} onClick={() => props.removeFromCart(props.item)}>
          -
        </Button>
        <Typography
          component='span'
        ...
        <Button variant='outlined' disabled={props.item.inCart >= 99} sx={{ minWidth: 5 }} onClick={() => props.addToCart(props.item)}>
          +
        </Button>
      </Box>
      <Box component='div' display='flex'>
        <Typography component='span' width={!props.isMobile ? '70px' : 'fit-content'} pr={1}>
          In Bag:
        </Typography>
